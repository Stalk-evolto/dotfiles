...
(define (module-used-interfaces name)
  (module-uses (resolve-module name)))

(define %libgit-module-names
  (map module-name (cdr (module-used-interfaces '(git)))))

(define* (update-git-mirror! #:optional
                             (base-path "/srv/git")
                             (user "git-daemon")
                             (group "git-daemon"))
            ...
            (with-imported-modules (source-module-closure
              ...
              #~(begin
                  (use-modules (git)
                               (ice-9 ftw)
                               (guix build utils))
                  (parameterize ((dir-list (find-dirs base-path)))
                    (for-each update-repository! dir-list))))))
  ...
  (match-record config <update-mirror-configuration>
                ...
                (list (shepherd-service
                       ...
                       (start #~(make-timer-constructor
                                 ...
                                 (lambda ()
                                   #$(apply update-git-mirror! (list path)))
